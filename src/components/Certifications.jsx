...
function Certifications() {
  ...
  return <Section id="certificacoes" title={lang === "pt" ? "Certificações" : "Certifications"} index="04"><div className="grid gap-4 md:grid-cols-2">{certificates.map((certificate) => <article key={certificate.title} className={`${cardClass} flex items-center justify-between gap-4 px-5 py-[18px]`}><div className="flex items-center gap-3.5">{certificate.image ? <img src={certificate.image} alt={certificate.provider} className="h-10 w-10 rounded-lg object-cover" /> : <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-lineStrong bg-panel font-mono text-[11px] font-semibold text-accentSoft">{certificate.initials}</span>}<div><h3 className="text-base font-semibold">{lang === "en" && certificate.titleEn ? certificate.titleEn : certificate.title}</h3><p className="mt-1 font-mono text-[11.5px] text-faint">{certificate.provider}</p></div></div>{certificate.href ? <a href={certificate.href} target="_blank" rel="noreferrer" className={ghostActionClass}>{lang === "pt" ? "Ver ↗" : "View ↗"}</a> : null}</article>)}</div></Section>;
}
